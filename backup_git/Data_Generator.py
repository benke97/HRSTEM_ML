...
class Data_Generator:
    ...
    def generate_data(self,number_of_images,save_name,benchmark=False):
        ...
        for i in range(self.number_of_images):
            ...
            point_set, point_set_without_augment = point_set_generator.generate_random_point_set()
            ...
            image, gaussian_ground_truth, exp_ground_truth, segmented_image,original_image = imager.generate_image(point_sets[i])
            if benchmark:
                benchmark_images, benchmark_ssim = imager.generate_benchmark_set(original_image.copy(),point_set)
                benchmark_sets.append(benchmark_images)
                benchmark_ssims.append(benchmark_ssim)

                #print(len(benchmark_images),len(benchmark_ssim),"pointsets:",len(point_sets))
                #if len(benchmark_images) != len(benchmark_ssim):
                #    raise ValueError("i dont feel so good")
                
                #for image_benchmark,ssim in zip(benchmark_images,benchmark_ssim):
                #        plt.figure()
                #        plt.imshow(image_benchmark, cmap="gray", origin='lower')
            ...
            unaltered_image = imager.generate_unaltered_image(point_set_without_augment)
            #print(np.max(image))
            plt.figure()
            ...
            plt.plot()
            plt.figure()
            plt.imshow(unaltered_image,cmap='gray',origin='lower')
            plt.axis('off')
            plt.plot()
            plt.figure()
        ...
        if benchmark:
            ...
            'benchmark_ssims':benchmark_ssims,
            'images': images,
    ...
    def visualize_one_image(self,file_name,index):
        with open(f'{file_name}.pkl', 'rb') as f:
            loaded_data = pickle.load(f)
        loaded_dataframe_list = loaded_data['dataframes']
        loaded_image_list = loaded_data['images']
        positions = loaded_dataframe_list[index][['x','y']].to_numpy()*128
        image = loaded_image_list[index]
        plt.subplot(1,2,1)  
        plt.imshow(image, cmap="gray", origin='lower')
        plt.title('Image')
        plt.subplot(1,2,2)
    ...
    def visualize_one_benchmark_image(self,file_name):
        with open(f'{file_name}.pkl', 'rb') as f:
            loaded_data = pickle.load(f)
        image = loaded_data['image'][2]
        positions = loaded_data['dataframe'][['x','y']].to_numpy()*128
        ...
        plt.subplot(1,2,2)
        plt.imshow(image, cmap="gray", origin='lower')
        plt.scatter(positions[:,0],positions[:,1],s=10,c='r')
        plt.axis('off')
    ...
    def generate_benchmark_images(self,save_name):
        ...
        point_set, point_set_without_augment = point_set_generator.generate_random_point_set(benchmark=True)
