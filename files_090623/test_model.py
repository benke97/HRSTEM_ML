...
with open('great_step.pkl', 'rb') as f:
    data = pickle.load(f)
...
analyzer = Analyzer()
analyzer.set_image_list(data["image"])
analyzer.set_gt_list(data['exponential_ground_truth'])
analyzer.set_pred_list(outputs)
analyzer.set_point_sets(data["dataframe"])
analyzer.set_SSIM_list(data["SSIM"])
...
def order_comparison(mean_values,standard_deviations,ssim):
    # Find where the ssim array increases compared to the previous value
    increases = np.diff(ssim) > 0

    # Pad the increases array with a False at the beginning to match ssim's shape
    increases = np.insert(increases, 0, False)

    # Create masks for where ssim doesn't increase
    no_increase = np.logical_not(increases)

    # Apply the masks to the arrays
    ssim = ssim[no_increase]
    mean_values = mean_values[no_increase]
    standard_deviations = standard_deviations[no_increase]
    return ssim, mean_values, standard_deviations
